...
export function renderCatalog(objects, elements, selectedIds) {
  const { grid, count, onSelect, onToggleCompare } = elements;
  ...
  grid.querySelectorAll('.catalog-card').forEach((card) => {
    const handler = (event) => {
      if (event?.target?.closest('.compare-toggle')) return;
...
export function renderFeaturedObject(objects) {
  ...
  const href = `${window.location.pathname.includes('/pages/') ? '../' : ''}pages/explore.html?object=${featured.id}`;
  ...
  if (card) {
    card.innerHTML = `
      ...
      <a class="text-link" href="${href}">Open full detail view</a>
    `;
  }
  ...
  if (spotlight) {
    spotlight.innerHTML = `
      ...
      <a href="${href}" class="text-link">Jump straight to this object</a>
    `;
  }
}
